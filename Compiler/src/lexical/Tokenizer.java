...
public class Tokenizer {
	...
	/**
	 * Getter for all the errors up till lexical phase
	...
	/**
	 * Finds the alphanumeric(only identifiers or reserved keywords but not
	 * literals, punctuation or other operators) token in the given input
	 * 
	 * @param input Input String
	 * @param lexemeBeginSt starting index of the lexeme to be generated
	 * @param lineNumber Line number in the source program
	 * @return token valid token in the input program
	 */
	public static Token alphaLexeme(String input, int lexemeBegin, Integer lineNumber) {
		...
		for (; forward < input.length();) {
			if (Character.isLetter(input.charAt(forward)) || Character.isDigit(input.charAt(forward))
					|| input.charAt(forward) == '_') {
				forward++;
			} else {
	...
	public static Token numericLexeme(String input, int lexemeBegin, Integer lineNumber) {
		...
		for (; forward < input.length();) {
			...
			if (Character.isDigit(input.charAt(forward))) {
				...
				// the control reaches here if current char is '.' and is not the last character
				// in input.

				forward++;
				if (Character.isDigit(input.charAt(forward)) && forward < input.length() - 1) {

					// the control reaches here if current char is a digit after '.' and is not the
					...
					for (; forward < input.length();) {
						...
						if (Character.isDigit(input.charAt(forward))) {
							...
							// the control reaches here if current char is 'e' and is not the last character
							// in input.
							...
							if (input.charAt(forward) == '0') {
								...
								for (; forward < input.length();) {
									...
									if (Character.isDigit(input.charAt(forward))) {
										...
										return new Token(TokenType.floatNum, input.substring(lexemeBegin, forward),
												lineNumber);
									}
								}
							...
							return new Token(TokenType.floatNum, input.substring(lexemeBegin, checkPoint + 1),
									lineNumber);
						}
					}
					...
					// the control reaches here if the previous character('.') is preceded by digit,
					// which is last in the input.

					forward++;
					return new Token(TokenType.floatNum, input.substring(lexemeBegin, forward), lineNumber);
				} else {

					// the control reaches here if the previous character('.') is not preceded by
					// required characters for the FLOAT token.
	...
	/**
	 * The main controller for lexical phase that calls the nextToken() function repeatedly to generate
	 * all the tokens for the given input. Once it hits the end of source program,
	 * it outputs all the tokens in the output file and encountered errors in the
	 * error file.
	 * 
	 * @throws IOException handles the I/O related interruptions
	 */
	public static void lexicalAnalyzer(String inputPath) throws IOException {

		System.out.println("Fetching input......");
		System.out.println("Recording tokens......");
		TreeMap<Integer, String> inputList = DataReadWrite.readInput(inputPath);
		map = new Util().getMap();
		...
		System.out.println();
		System.out.println("End of Lexical Phase");
		System.out.println();
	}
